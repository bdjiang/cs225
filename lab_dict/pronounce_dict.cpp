...
bool PronounceDict::homophones(const string& word1, const string& word2) const
{
    ...
    for(unsigned int l = 0; l < word1.length(); l++) {
      upperWord1[l] = toupper(word1[l]);
    }
    for(unsigned int l = 0; l < word2.length(); l++) {
      upperWord2[l] = toupper(word2[l]);
    }

    auto it1 = dict.find(upperWord1);
    auto it2 = dict.find(upperWord2);
