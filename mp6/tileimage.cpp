...
void TileImage::paste(PNG& canvas, int startX, int startY, int resolution, vector<PNG>& renderedTiles, bool found, int index) const {
    // If possible, avoid floating point comparisons. This helps ensure that
    // students' photomosaic's are diff-able with solutions
    PNG* scaled = new PNG(resolution, resolution);
    ...
    if (getResolution() % resolution == 0) {
        ...
        for (int x = 0; x < resolution; x++) {
            for (int y = 0; y < resolution; y++) {
                ...
                if (!found) {
                  HSLAPixel pixelToAdd = getScaledPixelInt(pixelStartX, pixelEndX, pixelStartY, pixelEndY);
                  *(canvas.getPixel(startX + x, startY + y)) = pixelToAdd;
                  *(scaled->getPixel(x,y)) = pixelToAdd;
                } else {
                  *(canvas.getPixel(startX + x, startY + y)) = *(renderedTiles[index].getPixel(x,y));
                }
            }
        }
        if (!found) {
          renderedTiles.push_back(*scaled);
        }
