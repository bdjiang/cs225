...
PNG MosaicCanvas::drawMosaic(int pixelsPerTile)
{
    ...
    vector<TileImage*> usedTiles;
    vector<PNG> renderedTiles;
    for (int row = 0; row < rows; row++) {
        ...
        for (int col = 0; col < columns; col++) {
            ...
            int index = 0;
            bool found = false;
            for (unsigned i = 0; i < usedTiles.size(); i++) {
              if (usedTiles[i] == images(row,col)) {
                found = true;
                index = i;
                break;
              }
            ...
            if (!found) {
              usedTiles.push_back(images(row, col));
            }
            images(row, col)->paste(mosaic, startX, startY, endX - startX, renderedTiles, found, index);
        }
    }
    if (enableOutput) {
        cerr << "\r" << string(60, ' ');
