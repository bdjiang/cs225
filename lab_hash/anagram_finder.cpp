...
bool AnagramFinder<Dict>::checkWord(const string& word, const string& test)
{
    ...
    std::vector<std::pair<char, int>> firstWord;
    std::vector<std::pair<char, int>> secondWord;

    for (size_t i = 0; i < word.length(); i++) {
      char wordChar = word[i];
      bool foundWord = false;
      for (size_t j = 0; j < firstWord.size(); j++) {
        if (firstWord[j].first == wordChar) {
          firstWord[j].second = firstWord[j].second + 1;
          foundWord = true;
          break;
        }
      }
      if (foundWord == false) {
        firstWord.push_back(pair<char, int> (wordChar, 1));
      }
    }

    for (size_t i = 0; i < word.length(); i++) {
      char wordChar = test[i];
      bool foundWord = false;
      for (size_t j = 0; j < secondWord.size(); j++) {
        if (secondWord[j].first == wordChar) {
          secondWord[j].second = secondWord[j].second + 1;
          foundWord = true;
          break;
        }
      }
      if (foundWord == false) {
        secondWord.push_back(pair<char, int> (wordChar, 1));
      }
    }
    ...
    for (size_t i = 0; i < firstWord.size(); i++) {
      char letterToCompare = firstWord[i].first;
      int numberToCompare = firstWord[i].second;
      bool letterExists = false;
      for (size_t j = 0; j < secondWord.size(); j++) {
        if (letterToCompare == secondWord[j].first) {
          letterExists = true;
          if (numberToCompare != secondWord[j].second) {
            return false;
          }
        }
      }
