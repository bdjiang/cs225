...
void HuffmanTree::buildTree(const vector<Frequency>& frequencies)
{
    ...
    while (singleQueue.size() + mergeQueue.size() > 1) {
      ...
      nodeOne = removeSmallest(singleQueue, mergeQueue);
      nodeTwo = removeSmallest(singleQueue, mergeQueue);

      int combinedFreq = nodeOne -> freq.getFrequency() + nodeTwo -> freq.getFrequency();
      TreeNode* newMerge = new TreeNode(combinedFreq);
      newMerge -> left = nodeOne;
      newMerge -> right = nodeTwo;

      mergeQueue.push(newMerge);
    }
...
HuffmanTree::TreeNode* HuffmanTree::readTree(BinaryFileReader& bfile)
{
    ...
    if (bfile.getNextBit()) {
      char nextByte = bfile.getNextByte();
      Frequency* newFreq = new Frequency(nextByte, 0);
